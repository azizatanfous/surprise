...
const randomQuestionGenerator = () => {
    ...
    // Keep generating random numbers until a unique question is found
    do {
      randomQuestionNum = randomNumHelperFunc(15);
    } while (questionsAsked.some(item => item[randomGameNum] === randomQuestionNum));
  
    // Update the questionsAsked array with the new question
    questionsAsked.push({ [randomGameNum]: randomQuestionNum });
  
    // Set currentQuestion to the new question
...
const gameOver = (index) => {
    if (index <= 0) {
      ...
      // Display different messages based on winning or losing
      let message = "";
      if (index === 999) {
        // Player wins
        message = `
          Congratulations, winner! Is it just me, or did winning that game make you even sexier? It must be that victorious glow. 🎉
        `;
      } else {
        // Player loses
        message = `
          Well, well, losing never looked so good on someone. Lucky for you, I find a little failure oddly attractive. 😄
        `;
      }
